...
| 30 | コード品質 | BIZ-006 | リファクタリング | BIZ-006-01 | カラー定義の共通化 | BIZ-006-01-01 | 契約ステージ・ステップのカラー定義を一元管理し、DRY原則に準拠する | SYS-006-01-01-01 | カラー定義共通化 | 1. src/constants/colors.tsを新規作成<br>2. 契約ステージ・ステップ・タスク優先度・タスクステータスのカラー定義を集約<br>3. 各コンポーネントから重複定義を削除し共通ファイルを参照 | 1. 8ファイルに渡る約237行のカラー定義重複を解消するため<br>2. カラー変更時に1箇所の修正で完結させるため<br>3. 保守性・一貫性の向上 | 共通カラー定義ファイル | 1. stageColors: 契約ステージ用（Badge表示、bg/text/border形式）<br>2. stepColors: 契約ステップ用（Badge表示）<br>3. dealStepColors: 案件一覧用（ステージ・ステップ両対応）<br>4. workflowStageColors: StatusWorkflow用（bg/border/text/active形式）<br>5. priorityColors: タスク優先度用<br>6. taskStatusColors: タスクステータス用 | 1. 各用途に応じたカラー形式を提供（Badge用、Workflow用）<br>2. 後方互換性のため旧ステージ・ステップのカラーも定義<br>3. 新規追加時もcolors.tsに集約 | src/constants/colors.ts | 中 | 完了 | - | REV-001 | 2025/12/26 |
| 31 | コード品質 | BIZ-006 | リファクタリング | BIZ-006-01 | 未使用コードの削除 | BIZ-006-01-02 | 使用されていないエイリアス・コンポーネントを削除し、コードベースをクリーンに保つ | SYS-006-01-02-01 | 未使用コード削除 | 1. constants/index.tsから非推奨エイリアス8件を削除<br>2. 未使用コンポーネントstatus-history-card.tsxを削除 | 1. 使用されていないコードがメンテナンス負荷となるため<br>2. 新規開発者の混乱を防ぐため<br>3. バンドルサイズの最適化 | ファイル削除・コード削除 | 1. 削除したエイリアス: CONTRACT_PHASE_LABELS, CONTRACT_STATUS_LABELS, STATUS_TO_PHASE, PHASE_STATUSES, NEXT_PHASE_FIRST_STATUS, ALL_CONTRACT_STATUSES, STATUS_DETAILS, STATUS_COMPLETION_MESSAGES<br>2. 削除したファイル: src/components/features/contracts/status-history-card.tsx | 1. grepで使用箇所がないことを確認してから削除<br>2. ビルド成功を確認 | src/constants/index.ts<br>src/components/features/contracts/status-history-card.tsx（削除） | 低 | 完了 | - | REV-001 | 2025/12/26 |
| 32 | ユーザー体験 | BIZ-002 | UI改善 | BIZ-002-02 | ステータスワークフローUI改善 | BIZ-002-02-01 | 契約詳細画面のステータスワークフローの視認性・操作性を改善する | SYS-002-02-01-01 | StatusWorkflow UI改善 | 1. ステージ進捗表示の視認性向上（サイズ拡大、影追加）<br>2. ステップ表示を非クリッカブルに変更（表示専用）<br>3. 完了ステージをグレー表示に統一<br>4. 現在ステージを強調表示（shadow-md） | 1. ステータスの視認性向上のため<br>2. 誤操作防止のためステップは表示のみに<br>3. 完了済みと進行中を視覚的に区別するため | StatusWorkflowコンポーネント改修 | 1. ステージ丸サイズ: w-10 h-10（以前はw-8 h-8）<br>2. チェックアイコン: h-5 w-5<br>3. 番号テキスト: text-sm font-bold<br>4. ラベル: mt-2 text-xs<br>5. 完了ステージ: bg-gray-300 + チェックアイコン白<br>6. 現在ステージ: stageColor.bg + shadow-md<br>7. 未到達ステージ: stageColor.bg + opacity-60<br>8. ステップ表示: div要素（button→div変更）<br>9. アクティブステップ: stageColor.active + text-white | 1. ステージ間をChevronRightで接続し進行方向を明示<br>2. 現在ステージにshadow-mdで立体感を付与<br>3. 完了ステージはグレーで進行済みを表現<br>4. ステップはボタンではなく表示のみにすることで誤操作防止 | src/components/features/deals/status-workflow.tsx<br>src/constants/colors.ts | 高 | 完了 | - | - | 2025/12/27 |
| 33 | コード品質 | BIZ-006 | ビルド修正 | BIZ-006-02 | 未使用インポートの削除 | BIZ-006-02-01 | ビルドエラーを引き起こす未使用インポートを削除する | SYS-006-02-01-01 | 未使用インポート削除 | 1. task-detail.tsxから未使用のArrowLeftインポートを削除<br>2. ビルド成功を確認 | 1. TypeScriptの未使用変数エラーによるビルド失敗を防ぐため<br>2. コードの清潔さを保つため | コード修正 | 1. lucide-reactからのArrowLeftインポートを削除<br>2. npm run buildで成功確認 | 1. ビルドエラーはデプロイ失敗に直結するため即時対応が必要 | src/components/features/tasks/task-detail.tsx | 高 | 完了 | - | - | 2025/12/27 |
...
外部コードレビューで指摘された問題への対応方針を整理。

#### 対応した問題

| 問題 | 重大度 | 対応内容 | 削減行数 |
|------|--------|----------|----------|
| DRY違反: カラーマッピング重複（8ファイル） | MAJOR | src/constants/colors.tsを作成し一元管理。6ファイルから重複定義を削除 | 約237行 |
| 未使用コード: 非推奨エイリアス | MINOR | constants/index.tsから8件のエイリアスを削除 | 約16行 |
| 未使用コンポーネント: status-history-card.tsx | MINOR | ファイルを削除 | 約150行 |

#### 対応しないと判断した問題

| 問題 | 重大度 | 対応しない理由 |
|------|--------|----------------|
| シークレットキーがGitリポジトリに露出 | CRITICAL | **誤検知**。.env.localは.gitignoreに含まれており、Git履歴に存在しない。git ls-filesでも追跡されていないことを確認済み |
| UIコンポーネントにDB操作が直接混入 | CRITICAL | **設計方針に合致**。Next.js App Router + Supabaseの構成では一般的なパターン。Repository層の導入はOver-engineering |
| クライアントサイドのみの認可チェック | CRITICAL | **Supabase RLSで対応済み**。デモモード用に全許可ポリシーを設定しているが、本番運用時にはRLSポリシーを適切に設定する設計（SYS-003-02-01-01参照） |
| 巨大コンポーネント（God Object） | MAJOR | **現時点では許容範囲**。指摘された5ファイルはそれぞれ単一の画面/機能を担当しており、責務は明確。分割による複雑性増加のリスクの方が大きい |
| フォーム送信パターンの重複 | MAJOR | **現時点では許容範囲**。4ファイル程度の重複であり、カスタムフックへの抽象化はOver-engineering |
| データアクセス層の欠如 | MAJOR | **設計方針として不採用**。Supabaseの型生成を活用しており、追加のDAL層は冗長 |
| 履歴記録の失敗を成功として返却 | MAJOR | **意図的な設計**。履歴記録がメイン機能の動作を妨げないようにするため。console.warnでログは残しており監視可能 |
| string型への過度な依存 | INFO | **低優先度**。定数ファイルで値を一元管理しており、実質的に型安全性は確保されている |
...
1. **未使用インポートの削除**
   - ファイル: `src/components/features/tasks/task-detail.tsx`
   - 問題: 未使用の`ArrowLeft`インポートによるビルドエラー
   - 対応: lucide-reactからのArrowLeftインポートを削除
   - 結果: ビルド成功、デプロイ完了

### 作業2: ステータスワークフローUIの改善

1. **レイアウトの調整（2日前の状態に戻しつつ改善）**
   ...
   - 変更内容:
     ...
     - ステージ間接続: 水平線 → ChevronRightアイコン
     - 現在ステージ: shadow-md追加
     - CardTitle: Infoアイコン付きツールチップ追加

2. **カラースキームの改善（version-6ブランチからマージ）**
   - 完了ステージ: bg-green-500 → bg-gray-300（グレー表示）
   - 現在ステージ: stageColor.bg + stageColor.border + shadow-md
   - 未到達ステージ: stageColor.bg + stageColor.border + opacity-60
   - ステップ表示:
     - アクティブ: stageColor.active + text-white + shadow-sm
     - 非アクティブ: bg-white + stageColor.border + stageColor.text

3. **ステップ表示の非クリッカブル化**
   - button要素 → div要素に変更
   - ステップ変更は「次のステップへ進む」ボタンで操作
   - 誤クリック防止とUI明確化

### 作業3: version-6ブランチのマージ

1. **マージ内容**
   - コミット: `fe9c41b feat: UI/UX改善6項目を実装`
   - 主な変更:
     - ステージ・ステップの色を統一（ステージ濃い・ステップ薄い）
     - 完了ステージはグレー表示
     - モダンなUI改善
...
1. **カラー定義の共通化（REV-001対応）**
   - 新規ファイル作成: `src/constants/colors.ts`
   - 6種類のカラー定義を集約:
     - stageColors: 契約ステージ用（Badge表示）
     - stepColors: 契約ステップ用（Badge表示）
     - dealStepColors: 案件一覧用（ステージ・ステップ両対応）
     - workflowStageColors: StatusWorkflow用（bg/border/text/active形式）
     - priorityColors: タスク優先度用
     - taskStatusColors: タスクステータス用
   - 修正したファイル:
     - contract-list.tsx: stageColors, stepColorsをインポートに変更
     - deal-list.tsx: dealStepColorsをインポートに変更
     - task-list.tsx: priorityColors, stepColors, taskStatusColorsをインポートに変更
     - status-workflow.tsx: workflowStageColorsをインポートに変更
     - task-detail.tsx: priorityColors, taskStatusColorsをインポートに変更
     - contract-task-card.tsx: priorityColors, taskStatusColorsをインポートに変更
   - 削減効果: 約237行の重複コードを削除

2. **未使用エイリアスの削除**
   - constants/index.tsから以下を削除:
     - CONTRACT_PHASE_LABELS（非推奨エイリアス）
     - CONTRACT_STATUS_LABELS（非推奨エイリアス）
     - STATUS_TO_PHASE（非推奨エイリアス）
     - PHASE_STATUSES（非推奨エイリアス）
     - NEXT_PHASE_FIRST_STATUS（非推奨エイリアス）
     - ALL_CONTRACT_STATUSES（非推奨エイリアス）
     - STATUS_DETAILS（非推奨エイリアス）
     - STATUS_COMPLETION_MESSAGES（非推奨エイリアス）

3. **未使用コンポーネントの削除**
   - 削除: `src/components/features/contracts/status-history-card.tsx`
   - 理由: どこからもインポートされていないことをgrepで確認

4. **ビルド確認**
   - `npm run build` が正常に完了することを確認

### 作業2: システムテストで発見されたバグ修正
