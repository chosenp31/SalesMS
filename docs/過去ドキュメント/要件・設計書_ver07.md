...
| 13 | データ管理 | BIZ-003 | 登録機能 | BIZ-003-01 | 活動履歴登録 | BIZ-003-01-03 | 案件に紐づく活動履歴を登録し、契約種別を選択可能にする | SYS-003-01-03-01 | 活動履歴登録機能 | 1. activitiesテーブルにcontract_idカラムを追加<br>2. 活動登録フォームで契約種別を選択必須に<br>3. 活動種別（電話/訪問/メール/オンラインMTG/その他）を選択<br>4. 議事録対応のためテキストエリアを拡大 | 1. どの契約に対する活動かを明確にするため<br>2. 活動履歴の分析・集計を容易にするため<br>3. 議事録などの長文記録に対応するため | ActivityFormコンポーネント拡張 | 1. contract_id: 契約UUID（必須）<br>2. activity_type: 活動種別（必須）<br>3. content: 活動内容（テキストエリア250px）<br>4. 契約選択を左上（優先度高）、活動種別を右に配置<br>5. マイグレーションでcontract_idカラムを追加 | 1. 契約種別は業務上必須の情報<br>2. 左上配置は視線の流れに沿った優先度表現<br>3. 250pxは議事録記入に適切なサイズ | src/components/features/activities/activity-form.tsx<br>supabase/migrations/20241224000000_add_contract_id_to_activities.sql | 高 | 完了 | - | - | 2025/12/23 |
